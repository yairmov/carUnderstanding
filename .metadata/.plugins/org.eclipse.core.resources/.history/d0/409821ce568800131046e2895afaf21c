...
def test_fg_utils():
  ...
  train_ann_file = '/usr0/home/ymovshov/Documents/Research/Code/3rd_Party/fgcomp2013/release/train_annos.txt'

  print 'before load'
  domain_meta = fgu.read_domain_meta(domain_meta_file)
  class_meta = fgu.read_class_meta(class_meta_file)
  train_annos = fgu.read_training_data(train_ann_file)
  print 'after load'
  ...
  print '-------------'
  print train_annos.dtypes

  print 'Showing the first two lines of train_annos'
  print '------------------------------------------'
  print train_annos.iloc[0:2,:]
